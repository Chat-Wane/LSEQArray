...
Base.prototype.getInterval = function(p, q, level){
    ...
    var prev = Math.floor(p * Math.pow(2, bitBaseSum - prevBitLength));
    // #1b process the next digit
    var next = Math.floor(q * Math.pow(2, bitBaseSum - nextBitLength));
    
    // #2 process the particular case: q<p at the targeted level
    if (next - prev < 0) {
	// #2a: look after the common root
	var i = 1;
	var sumBitI = Base.getSumBit(i);
	while ((Math.floor(prev / Math.pow(2, bitBaseSum - 1 - sumBitI))  ==
		Math.floor(next / Math.pow(2, bitBaseSum - 1 - sumBitI))) &&
	       (bitBaseSum - 1 - sumBitI >= 0)) {
	    ++i;
	    sumBitI = Base.getSumBit(i);
	}
	// #2b: add one
	next = Math.floor(next / Math.pow(2, sumBitI -
					  1 - Base.getSumBit(i - 1))) + 1;
	// #2c: append missing zeros
	next = Math.floor(next * Math.pow(2, bitBaseSum - sumBitI + 1));
    }
    return next - prev - 1;
};
